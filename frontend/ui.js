...
async function fetchCurrentWeekSessions(startDate, endDate) {
...
function renderHistoricoScreen() {
  ...
  screen.innerHTML = `
    <div class="app-shell history-shell">
      <header class="topbar">
        <div><div class="brand">Diário</div><small style="color:var(--muted);">Carregando sua semana de treino...</small></div>
      </header>
      <section class="screen-card analytics-loading-card" aria-live="polite">
        <span class="analytics-loading-mark"></span>
        <strong>Carregando sua semana</strong>
        <small>Buscando seus treinos salvos.</small>
      </section>
    </div>
    ${renderNavBar()}
  `;
  ...
  (async () => {
    try {
      ...
      const sessions = await fetchCurrentWeekSessions(start, end);
        ...
        <div class="app-shell history-shell">
          <header class="topbar">
            <div>
              ...
              <small style="color:var(--muted);">Sua semana de treino</small>
            </div>
          </header>
      ...
      screen.innerHTML = `
        <div class="app-shell history-shell">
          <header class="topbar">
            <div>
              <div class="brand">Diário</div>
              <small style="color:var(--muted);">Não foi possível carregar sua semana</small>
            </div>
          </header>
          <div class="screen-card analytics-error-card">
            ...
            <button class="secondary-btn" id="retry-history-btn">Tentar novamente</button>
          </div>
        </div>
        ${renderNavBar()}
      `;
      screen.querySelector('#retry-history-btn')?.addEventListener('click', () => renderHistoricoScreen());
    }
  })();
}
        ...
        <div class="series-table">
          ...
          ${rowsWithHistory.map((row, index) => `
            <div class="series-row-item ${index === 0 ? 'selected' : ''} ${row.type === 'T' ? 'work' : row.type === 'P' ? 'prep' : 'warm'} ${row.is_completed ? 'completed' : ''}" data-set-index="${index}">
              ...
              <div class="workout-input-wrap"><div class="input-label-row"><span>REPS</span></div><input aria-label="Repetições da série ${index + 1}" class="set-input" data-field="reps" data-index="${index}" inputmode="numeric" type="number" min="0" placeholder="${row.previous?.reps ?? ''}" value="${row.reps}">${row.previous ? `<button type="button" class="previous-value-link" data-fill-previous="${index}" data-fill-field="reps">Anterior: ${row.previous.reps ?? 0}</button>` : '<span class="previous-value-empty">Sem histórico</span>'}</div>
              <div class="workout-input-wrap"><div class="input-label-row"><span>CARGA</span></div><input aria-label="Carga da série ${index + 1}" class="set-input" data-field="weight_kg" data-index="${index}" inputmode="decimal" type="number" min="0" step="0.5" placeholder="${row.previous?.weight_kg ?? ''}" value="${row.weight_kg}">${row.previous ? `<button type="button" class="previous-value-link" data-fill-previous="${index}" data-fill-field="weight_kg">Anterior: ${row.previous.weight_kg ?? 0} kg</button>` : '<span class="previous-value-empty">Sem histórico</span>'}</div>
